...
caracter db 1 dup ('*') ;símbolo a mostrar
pos_x db 1 dup(1)  ;posición  virtual en X
pos_y db 1 dup(1)  ;posición virtual en y
...
ultimoleido db 1 dup(' ') ; guardo el ultimo carácter para regresarlo 
;============================================================================
.code
main:   
    mov ax,@data
    ...
    mov al,es:[bx]          ;busco la posición 0
    mov ultimoleido,al      ;guardo lo para regresar ese valor 
;==========ciclo_principal mantener código ==================================
ciclo_principal:        
    call pinta              ; llamo a método que pinta              
    call correr             ; método con validaciones 
    call pausa              ; método pausa  
    call finalizo           ; método que finaliza
...
finalizo:
    ...
    mov ah,0Bh              ;verifico si insertaron algo por teclado
...
pinta:  
    ...
    call optener_posi       ;obtengo la nueva posición a pintar 
...
limpia:
    ...
    mov al,ultimoleido      ; guardo el carácter a imprimir 
    mov es:[bx],ax          ; retorno el carácter 
    pop dx cx ax            ; retornamos las variables del cache 
    ret
;==========guarda Valor de la  pantalla==================================
guardaValor:
    push ax cx dx           ; guardo en cache la data para no molestar otros programas
...
cambiar_color:
    ...
    cmp cambiocolor,0       ; comparo con el indicador 
...
correr:
    ...
    jmp next1               ; si no es igual consulto siguiente pregunta 
    finll:                  
    ...
    jmp next3               ; si no es igual consulto siguiente pregunta 
    finll2:
    mov cambiocolor,1       ; enciendo el indicador
    ...
    jmp fincorrer           ; si no es igual consulto siguiente pregunta 
    finll3:
    mov cambiocolor,1       ; enciendo el indicador
    mov mov_y,1             ; cambio el sentido
...
ret
;====================Retorno la posición en el arreglo================
optener_posi:
    push ax cx dx           ; guardo en cache la data para no molestar otros programas
    ...
    mov bl,al               ; si es cero le agrego a bl la posición de X
    jmp fin_optener_posi    ; fin del método
    continua:               
    mov cl,ah               ; fin del ciclo es ah
    label1:     
    add bx,160              ; añado la posición
    loop label1             ; repito ah veces
    xor ax,ax               ; reinicio ax
